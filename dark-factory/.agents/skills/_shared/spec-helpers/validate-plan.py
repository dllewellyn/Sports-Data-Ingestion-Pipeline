...
STEP_FIELDS = [
    "goal", "spec trace", "red", "implementation",
    "green criterion", "guardrails to satisfy", "self-review checkpoint",
]
    ...
    for tbl in doc.tables(region):
        for row in tbl["rows"]:
            ...
            if re.search(r"\bgap\b", cell):
                errors.append("convention audit has an unresolved GAP row: "
...
def _check_steps(body, errors):
    ...
    step_starts = [(i, ln) for i, ln in enumerate(lines)
