...
case "${cmd}" in
  init)
    ...
    # Reuse an active run ONLY on a genuine resume: a --feature-dir was supplied
    # and the recorded run is unclosed and either hasn't claimed a feature yet
    # (late-bind adoption) or claims the SAME feature. A new-feature intake (no
    # --feature-dir, because the spec dir doesn't exist yet) must NOT reuse — it
    # forks a fresh trace, otherwise every feature collapses onto the first run
    # whose `close` was missed.
    if [[ -f "${CURRENT}" ]]; then
...
PY
    ...
    # Forking a new trace: finalize any stale, still-`running` run first so its
    # root span is emitted instead of being silently abandoned.
    if [[ -f "${CURRENT}" ]]; then
      prev_status="$(py - "$CURRENT" <<'PY'
import json, sys
try:
...
PY
      ...
      if [[ "${prev_status}" == "running" ]]; then
        "${0}" close --status error >/dev/null 2>&1 || true
      fi
    fi
...
PY
  ...
  attach)
    ...
    # (Binding the feature dir INTO the run is done by `init --feature-dir`, not
    # here — attach also runs during mint, when feature.json may still belong to a
    # prior feature, so it must never adopt a feature dir.)
