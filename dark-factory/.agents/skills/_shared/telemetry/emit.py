...
def _feature_label(explicit=None):
    """Human-readable feature id for the active run — the basename of the bound
    feature dir (e.g. ``002-subdir-install-layout``), or '' until specification
    binds the run to a feature. Promoted to a Loki index label and set as a span
    attribute so the dashboard can offer a `feature` dropdown that lists features,
    not just opaque run ids. Auto-resolved from the active-run pointer so every
    emit path (CLI and hooks) carries it without threading it through each caller."""
    if explicit:
        return explicit
    fd = (current_context().get("feature_dir") or "").rstrip("/")
...
def send_span(
    *,
    ...
    span = {
        ...
        "attributes": _attrs(
            [("feature.run_id", run_id), ("feature", _feature_label(feature))]
            + list(attrs)
        ),
...
def send_logs(*, trace_id, run_id, records, feature=None, debug=False, strict=False):
    """records: iterable of dicts {body, severity, attrs: [(k, v)]}.

    Label model: `run_id` and `feature` are RESOURCE attributes (constant per run)
    promoted to Loki index labels (Loki only allows index_label on resource attrs)
    so the dashboard can offer a `feature` dropdown (lists features) chained to a
    `run_id` dropdown (lists that feature's runs); `service.name` is the other
    selector label. Everything else (event_type, agent_id, role, phase, git_*)
    rides as flat structured metadata, filterable with `| key="…"`."""
    log_records = []
    for r in records:
        ...
        records=[
            {
                ...
                "attrs": [("event_type", args.type), ("phase", args.phase)]
                + _kv_list(args.attr),
            }
        ],
        debug=args.debug,
...
def cmd_label_next(args):
    ...
    try:
        ...
        with open(os.path.join(queue_dir, "pending-roles.jsonl"), "a", encoding="utf-8") as fh:
            fh.write(json.dumps({"role": args.role, "phase": args.phase, "ts_ns": time.time_ns()}) + "\n")
