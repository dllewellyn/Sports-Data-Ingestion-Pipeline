...
5. **Bind the telemetry run to this feature** (best-effort — silent no-op if no run is active or the
   telemetry stack is down). This MUST happen here, immediately after the `feature.json` write, so the
   binding can never be skipped: it gives the active run a feature identity (so the Grafana *Feature
   runs* dashboard groups it under this feature, not an opaque run id, and a later resume reuses the
   same run) and mirrors the run ids back into `feature.json`:
   ```bash
   bash .agents/skills/_shared/telemetry/run-context.sh init --feature-dir specs/NNN-<slug>
   bash .agents/skills/_shared/telemetry/run-context.sh attach
   ```
