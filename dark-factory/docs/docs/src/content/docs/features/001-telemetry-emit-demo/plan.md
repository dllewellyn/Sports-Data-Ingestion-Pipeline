...
- **Implementation:** After the harness convention rule is approved+committed (Open Q B2), add a minimal root `pyproject.toml` with `[tool.pytest.ini_options] testpaths = ["tests"]` and a `[tool.ruff]` lint set `E,W,F,I,UP,B,C4,SIM`; create `tests/`. Add `pytest`/`ruff` as dev deps via the repo's package manager (`uv`), respecting any lock; do not swap tooling. No runtime dep added.
...
- **Implementation:** After the package-marker rule is approved+committed (Open Q B1), add an empty `telemetry/__init__.py` (no exports needed).
...
- **Convention gate edge:** S0 depends on B2 approval, S1 on B1 approval (rules committed before the
  code that relies on them). If approval is withheld, S0/S1 (and downstream) are not "ready".
...
- **B1 (BLOCKER — approval needed): commit the package-marker convention rule.** Drafted (non-interactive,
  not yet committed): *"ALWAYS add an `__init__.py` before shipping a `python -m <pkg>.<module>` entry
...
- **B2 (BLOCKER — approval needed): commit the pytest-harness + lint convention rule and create
  `pyproject.toml`.** Drafted: *"ALWAYS place pytest tests in top-level `tests/` named `test_*.py`,
