...
2. **Name the guardrails** that protect this change and, for each, add a "verify it's in place" check to the plan: pre-commit hook installed + `ruff check`/`ruff format` clean; dbt tests present and run via `dbt build`; Pydantic/Pandera validation at the boundary; idempotency / re-run safety; OTel span emitted where the spec implies it; the repo's *Non-obvious constraints* (single-writer DuckDB, prefixed dbt asset keys, no `from __future__ import annotations` in asset modules) respected.
