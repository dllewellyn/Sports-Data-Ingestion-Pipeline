...
the `implementor` skill can complete it. This command MUST run only after
the `implementor` skill has run on the current `tasks.md`, and after the `tasks` skill has produced a complete `tasks.md`.
...
prerequisite skill to run (the `specification` skill for a missing spec, the `plan` skill for a missing plan,
the `tasks` skill for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running the `implementor` skill to complete them; note that a follow-up converge
