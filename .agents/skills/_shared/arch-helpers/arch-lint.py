...
def main(argv):
    ...
    ap.add_argument(
        "--src",
        default="src/data_platform",
        help="package root to scan (default: src/data_platform)",
    )
    ap.add_argument(
        "--edge",
        default="assets/bronze.py",
        help="the one file allowed outbound network calls (path relative to --src)",
    )
    ap.add_argument(
        "--config",
        default="config.py",
        ...
        if top in NETWORK_MODULES and os.path.normpath(path) != edge:
            findings.append(
                (
                    ...
                    lineno,
                    "Rule 1",
                    f"network module '{name}' imported outside the edge ({os.path.relpath(edge)})",
                )
            )
...
def _report(src, findings):
    if not findings:
        print(
            f"OK: no mechanical ARCHITECTURE.md violations under {src} "
    ...
    print(
        f"\n{len(findings)} finding(s) — confirm each in context; rules 2/6/8 are not checked here"
    )
    return 1
