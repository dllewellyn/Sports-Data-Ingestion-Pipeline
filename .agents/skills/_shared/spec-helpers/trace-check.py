...
def main(argv):
    ...
    if uncovered:
        errors.append(
            "spec FR/SC absent from the plan's Traceability: "
    ...
    orphans = [
        s for s in plan_steps if s not in setup_steps and not re.search(rf"\b{s}\b", plan_trace)
    ]
    if orphans:
        errors.append(
            "non-setup plan steps absent from the plan's Traceability: "
            + ", ".join(sorted(orphans, key=_idsort))
        )

    if tasks_body is not None:
        uncovered_steps = [s for s in plan_steps if not re.search(rf"\b{s}\b", tasks_body)]
        if uncovered_steps:
            errors.append(
                "plan steps with no task in tasks.md: "
...
def _setup_steps(body):
    ...
    starts = [
        (i, re.search(r"\bS\d+\b", ln).group(0))
...
def _report(paths, spec_items, plan_steps, tasks_body, errors):
    ...
    if errors:
        ...
        return 1
    print(
        "OK: traceability closed (every FR/SC reaches a step; every step traces back"
