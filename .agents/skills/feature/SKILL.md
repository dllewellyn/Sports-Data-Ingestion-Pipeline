...
6. **Open the run trace (telemetry).** Run `bash .agents/skills/_shared/telemetry/run-context.sh init`
   (add `--feature-dir <dir>` when resuming a known feature) to mint-or-reuse this run's `run_id` +
   `trace_id` — every phase, sub-agent, gate, and commit is correlated to it for the Grafana
   *Feature runs* dashboard, and a resume reuses the same trace. This is best-effort: if the telemetry
   stack is down it is a silent no-op and the run proceeds normally. Once `specification` has written
   `feature.json`, run `run-context.sh attach` to mirror the ids into it. Then record the request that
   started the run so the dashboard shows it:
