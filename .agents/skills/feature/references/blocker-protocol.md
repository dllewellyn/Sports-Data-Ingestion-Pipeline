...
4. **A destructive or irreversible action** — anything that deletes/overwrites data the agent didn't create, rewrites history, pushes, or touches something outside the feature's scope. Always confirm first (this is a standing rule, not specific to Feature).
