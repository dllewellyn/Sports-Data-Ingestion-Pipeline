...
1. Run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run the `specification` skill or verify feature branch environment.
...
- If spec file missing, instruct user to run the `specification` skill first (do not create a new spec here).
...
- If any Outstanding or Deferred remain, recommend whether to proceed to the `plan` skill or run `/speckit-clarify` again later post-plan.
