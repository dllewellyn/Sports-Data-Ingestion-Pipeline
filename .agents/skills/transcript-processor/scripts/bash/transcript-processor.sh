#!/usr/bin/env bash
# Discovery and status helper for the transcript-processor agent skill.
...
Commands:
  list     Print pending new_* files (one per line, tab-separated: sha  flat_path  source_path)
  status   Show done vs pending counts per commit

Options:
  --commit <short_sha>   Scope to a single commit dir
  --filter <substring>   Only files whose flat path contains this
...
# For a new_* file, compute its extracted_*.json sibling path
extracted_for() {
  local new_file="$1"
  local dir base stem
  ...
  stem="${base#new_}"
  echo "${dir}/extracted_${stem}.json"
}

# Convert flat filename back to original path for display (transcripts__foo.txt -> transcripts/foo.txt)
original_path() {
