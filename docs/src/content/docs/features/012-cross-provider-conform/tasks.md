---
title: "Tasks: Cross-Provider Conform — Symmetric Resolve-or-Mint for Every Provider"
---
...
- [X] T030 [S11] Cross-provider de-dup + full-run parity verification: run `dagster job execute -j matchbook_conform_job` (queued/daemon path — no `DagsterCodeLocationNotFoundError`) to completion; `dbt build` over intermediate+marts green; `int_match` has exactly ONE row for the shared fixture (keep-one); parity test green (U12, U17). No new product code. Depends on T028, T029.
- [X] T031 [S12] Write the docs red state: after planning the edits, run `grep -rn "try_read_parquet\|conform lives in matchbook\|canonical.*ESPN-only\|models/silver/canonical" CLAUDE.md ARCHITECTURE.md ERD.md "data flows.md" dbt/data_platform/models/intermediate/int_match.sql` and record every stale hit that must be removed (U19). Fails (stale text present) before edits.
- [X] T032 [P] [US1] [S12] Ripple `ARCHITECTURE.md`: describe conform as a symmetric cross-provider layer in `src/data_platform/conform/`, the four additions files, and correct any `models/silver/canonical/` → `models/intermediate/` staleness. `[P]` with T033, T034 (disjoint doc files). Depends on T030.
- [X] T033 [P] [S12] Ripple `ERD.md`: document the `league_aliases` seed + the four `<provider>_canonical_*_additions.parquet`, and correct `models/silver/canonical/` staleness. `[P]` with T032, T034. Depends on T030.
- [X] T034 [P] [S12] Ripple `data flows.md`: describe the symmetric conform flow (sources → bronze → silver canonical union of all providers → four additions files). `[P]` with T032, T033. Depends on T030.
- [X] T035 [S12] Fix stale `try_read_parquet` comments in `dbt/data_platform/models/intermediate/int_match.sql` (it is `read_parquet`, errors on missing file — FR-014). Depends on T030. Serial with US1's `int_match` work — NOT `[P]` with T024.
- [X] T036 [S12] Ripple `CLAUDE.md`: describe symmetric cross-provider conform (remove "canonical from ESPN"/"conform lives in matchbook"/stale UNION-ALL notes) AND land the two OQ-A-approved conventions drafted in T004/T005 into *Non-obvious constraints* (the four-file additions convention + the `league_aliases`/`_seeds.yml` convention). Green: the T031 grep finds no stale text; `uv run pre-commit run --all-files` clean on changed files (pre-existing SIM105 in the MCP server is not ours). Depends on T032, T033, T034, T035. Serial with the other doc tasks that also gate on the same grep — NOT `[P]`.
- [X] T037 [S11] Validate `quickstart.md`: walk its 8 runnable validation scenarios end-to-end and confirm each passes against the built pipeline. Depends on T030, T036.
...
> Appended by `speckit-converge` (2026-07-01). Assessed the committed codebase against spec/plan/tasks. One CRITICAL missing-code gap and one LOW comment-contradiction found; everything else in scope is genuinely built and green (53 conform tests pass, `dbt parse` exit 0, relocation total, `md5('matchbook_football')` gone, both matchbook link FK tests present, all four `int_*` models union both providers' additions, `league_aliases` seed carries the parity mapping `matchbook,15|Soccer-1234 → md5('eng.1')`, resolver consulted by the mint path).

- [X] T038 [US5] [S8-followup] CRITICAL: The four `football_data_canonical_{match,team,league,season}_additions.parquet` files are never bootstrap-written by any pipeline/asset — `bootstrap_additions_files("football_data", ...)` (`src/data_platform/conform/__init__.py:48`) exists but is invoked ONLY from `tests/conform/test_football_data_scaffold.py`, never from `matchbook_conform` (`assets/intermediate/matchbook_conform.py:44-71` bootstraps only the four matchbook files + t60). `int_match`/`int_team`/`int_league`/`int_season` each `read_parquet(...football_data_canonical_*_additions.parquet)` against a LITERAL missing path, which DuckDB errors on (verified: `read_parquet` on a missing literal file raises `IOException`, NOT zero rows). So a real `matchbook_conform` → `dbt build` run fails the four `int_*` models before any catalog interaction (a plain local-file read), independent of the DuckLake infra outage. Bootstrap the four football_data files empty in the pipeline path before the `int_*` models run — e.g. call `bootstrap_additions_files("football_data", settings.matchbook_canonical_additions_dir)` from the `matchbook_conform` asset (or the appropriate pre-dbt asset), reusing the existing shared helper (the "reused by both providers" intent of T028). Verify: with NO football_data data present, the four files exist as empty Parquet before dbt runs and `dbt build --select int_match int_team int_league int_season` stays green (satisfies SC-006 / FR-016 / E4 for football_data, closing FR-010's "materialise unchanged when the (bootstrap-written empty) files contribute no rows"). per FR-016 / FR-010 / SC-006 / E4 (missing)
- [X] T039 [US1] [S12-followup] LOW: The `read_parquet`-on-missing-file behaviour is documented CONTRADICTORILY across the four canonical models. `int_match.sql:73,91,117` correctly say "read_parquet REQUIRES the file to exist (it errors if absent)"; but `int_team.sql:54`, `int_league.sql:29`, and `int_season.sql:32-33` say the opposite ("read_parquet returns zero rows (not an error) when the file is absent (E11)") and cite an "E11" that does not exist in the spec edge-case table (E1–E10). The zero-rows claim is factually wrong for a literal path (verified above) and directly contradicts FR-016 / E4 / T035 / CLAUDE.md; it also masks the T038 defect by implying the missing football_data files are harmless. Correct the `int_team`/`int_league`/`int_season` comments to match `int_match.sql`'s accurate wording (errors on missing file → bootstrap-empty required) and drop the phantom "E11" reference. Comment-only; no logic change. per FR-014 / E4 (contradicts)
