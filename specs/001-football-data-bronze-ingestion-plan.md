...
- **A-plan-4** — The league registry is an in-repo typed Python constant (the build-time agreed list per spec A1), not external config; its exact contents are finalised in S2 from the investigation (11 main / 19 extra). **Resolved in S2:** a live enumeration found **16** extra leagues (not ~19); the registry reflects the real 16. The S2 red test asserts 16 accordingly.
