...
def discover_soccer_leagues() -> list[dict]:
    ...
    page = 1
    while True:
        data = _get(f"https://sports.core.api.espn.com/v2/sports/soccer/leagues?limit=100&page={page}")
...
def fetch_matches(sport_slug: str, league_id: str, league_name: str,
                  start_date: str, end_date: str) -> list[Match]:
...
def run_spike(sample_only: bool = True):
    ...
    for sport_slug, sample_ids in [("soccer", SAMPLE_SOCCER), ("rugby", SAMPLE_RUGBY)]:
        ...
        if sample_only:
            ...
            leagues = discover_soccer_leagues() if sport_slug == "soccer" else discover_rugby_leagues()
        ...
        for league in leagues:
            ...
            # Get league name
            if sport_slug == "soccer":
                detail = _get_safe(f"https://sports.core.api.espn.com/v2/sports/soccer/leagues/{lid}")
            else:
                detail = _get_safe(f"https://sports.core.api.espn.com/v2/sports/rugby/leagues/{lid}")
            ...
            for season in seasons:
                matches = fetch_matches(sport_slug, lid, league_name, season["start"], season["end"])
    ...
    for m in all_matches[:5]:
        print(f"  [{m.sport}] {m.league_name} | {m.home_team_name} vs {m.away_team_name} | {m.kickoff_utc} | {m.status}")
