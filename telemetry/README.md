...
- `run-context.sh` mints a `run_id` + W3C `trace_id` at run start (Phase 0), persisted
  to `temp/telemetry/current.json` (and mirrored into `.specify/feature.json`). A
  resume reuses the same ids, so the trace continues rather than forking.
...
**Using it:** open the **Feature Runs** dashboard, pick a `run_id`. The trace panel
...
- **Feature spans vs Claude Code logs**: feature-run spans/events carry
  ...
  `run_id` is a Loki index label, everything else is structured metadata.
```
